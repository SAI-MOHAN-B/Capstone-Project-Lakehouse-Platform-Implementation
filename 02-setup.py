...
class SetupHelper():
    ...
    def assert_table(self, table_name):
        assert spark.sql(f"SHOW TABLES FROM {self.catalog}.{self.db_name}") \
    ...
    def validate(self):
        import time
        start = int(time.time())
        print(f"starting setup validation....")
        assert spark.sql(f"SHOW DATABASES FROM {self.catalog}") \
    ...
    def cleanup(self):
        if spark.sql(f"SHOW DATABASES FROM {self.catalog}").filter(f"databaseName == '{self.db_name}'").count() == 1:
            print(f"Dropping the database {self.catalog}.{self.db_name}......")
            spark.sql(f"DROP DATABASE {self.catalog}.{self.db_name} CASCADE")
            print("Done......")
        print(f"Deleting {self.landing_zone}")
        dbutils.fs.rm(self.landing_zone,True)
        print("Done")
        print(f"Deleting {self.checkpoint_base}.....")
