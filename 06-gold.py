...
class Gold():
    ...
    def upsert_workout_bpm_summary(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_delta = (spark.readStream
                    ...
                    .select("workout_id", "session_id", "user_id", "age", "gender", "city", "state", "min_bpm",
                            "avg_bpm", "max_bpm", "num_recordings"))
        ...
        if once == True:
            ...
            return stream_writer.trigger(processing_time=processing_time).start()

    def upsert(self, once=True, processing_time="5 seconds"):
        import time
        start = int(time.time())
        print("Executing Gold Layer Upsert")
        self.upsert_workout_bpm_summay(once, processing_time)
        if once:
    ...
    def assert_rows(self,location,table_name,sets):
        print(f"validating records in {table_name}...")
        ...
        assert expected_rows ==  actual_rows, f"Expected data mismatches with actual data in {table_name}"
        print(f"Expected data matches with the actudal data in {table_name}: success")

    def validate(self,sets):
        import time
        start = int(time.time())
        print("validating gold layer records.....")
        self.assert_rows("7_gym_summary","gym_summary",sets)
        if sets > 1:
                self.assert_rows("workout_bpm_summary",2)
        print(f"Gold layer validation will be completed in {int(time.time())-start} seconds")
